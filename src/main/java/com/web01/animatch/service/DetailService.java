 ...
 private void setSearchDetailData(HttpServletRequest request, String userId) throws MyException {
  ...
  try {
   if (!StringUtils.isNumeric(userId)) {
    throw new MyException("msg.error.003",
      this.messageService.getMessage(MessageService.MessageType.ERROR, "003", "パラメータのユーザID"));
   }
   ...
   case OWNER:
    ...
    if (trimmerInfo == null) {
     throw new MyException("msg.error.008",
       this.messageService.getMessage(MessageService.MessageType.ERROR, "008"));
    }
    ...
    for (TrimmerInfoBusinessHours trimmerInfoBusinessHours : trimmerInfoBusinessHoursList) {
     ...
     if (!StringUtils.isEmpty(businessDay) && startBusinessTime != null && endBusinessTime != null) {
      trimmerInfoBusinessHours.setDisplayBusinessHours(
        this.propertiesService.getValue(PropertiesService.WEEKDAY_KEY_INIT_STR + businessDay));
