  ...
  <div class="container-fluid">
   ...
   <div class="row">
    <div class="col">
     <c:if test="${searchType == firstTypeKeyInitEnd}">
      <h1 class="mt-2">
       <img src="/animatch/images/icon_pad.png" alt="犬アイコン" class="main-search-header-img">飼い主専用
      </h1>
     </c:if>
     <c:if test="${searchType == secondTypeKeyInitEnd}">
      <h1 class="mt-2">
       <img src="/animatch/images/icon_beauty_salon.png" alt="お店アイコン" class="main-search-header-img">お店専用
      </h1>
     </c:if>
     <p>
      <span><c:out value="${searchCount}" /></span>件見つかりました。
     </p>
    </div>
   </div>
    ...
    <div class="col-xl-9 ml-0 mb-3 px-0 main-search-right">
     ...
     <div class="main-search-right-form">
      ...
      <div class="input-group my-3 col-11">
       <input type="text" id="search-contents" class="form-control"
       placeholder="${searchType == firstTypeKeyInitEnd?'店舗名、アピールポイントについて、キーワードを入力':'ペット名、備考について、キーワードを入力' }"
       value="<c:out value="${searchForm.searchContents}"/>">
       ...
       <div class="row main-search-right-list is-show-details-owner">
        ...
        <div class="col-9">
         ...
         <h6 class="mt-2">
          ...
          <span class="ml-2">ID:<c:out value="${trimmerInfo.userId}" /></span>
         </h6>
          ...
          <c:if test="${not empty trimmerInfo.trimmerInfoBusinessHoursList}">
           <c:forEach items="${trimmerInfo.trimmerInfoBusinessHoursList}" var="trimmerInfoBusinessHours">
            <div class="col-sm-1 p-0">
             <span class="mr-1">
             <c:out value="${trimmerInfoBusinessHours.displayBusinessHours}" /></span>
       ...
       <div class="row main-search-right-list is-show-details-trimmer">
        ...
        <div class="col-9">
         ...
         <h6 class="mt-2">
          ...
          <span class="ml-2">ID:<c:out value="${ownerInfo.userId}" /></span>
         </h6>
    ...
    <ul class="pagination justify-content-center pb-3 mb-0">
     ...
     <c:choose>
      <c:when test="endPageNum < 2">
       <li class="page-item">
        <a class="page-link" href="${requestURLWithParam}${oneStr}${paramStartPage}${oneStr}">${oneStr}</a>
       </li>
      </c:when>
      <c:otherwise>
       ...
       <c:forEach begin="${displayStartPageIndexNum}" end="${displayEndPageIndexNum}" varStatus="status">
        ...
        <c:choose>
         <c:when test="${searchType == firstTypeKeyInitEnd}">
          <li id="${pageItemIDInitName}${pageLinkNumStr}" class="page-item">
          <a class="page-link"
           href="${requestURLWithParam}${pageLinkNumStr}${paramStartPage}${displayStartPageIndexNum}${paramSearchContents}<c:out value="${searchForm.searchContents}"/>${paramUserId}<c:out value="${searchForm.userId}"/>${paramPrefectures}<c:out value="${searchForm.prefectures}"/>${paramCities}<c:out value="${searchForm.cities}"/>${paramBusinessHoursWeekday}<c:out value="${searchForm.businessHoursInputValue}"/>${paramBusinessHoursStartTime}<c:out value="${searchForm.businessHoursStartTime}"/>${paramBusinessHoursEndTime}<c:out value="${searchForm.businessHoursEndTime}"/>">${status.index}</a></li>
         </c:when>
         <c:when test="${searchType == secondTypeKeyInitEnd}">
          <li id="${pageItemIDInitName}${pageLinkNumStr}" class="page-item">
          <a class="page-link"
           href="${requestURLWithParam}${pageLinkNumStr}${paramStartPage}${displayStartPageIndexNum}${paramSearchContents}<c:out value="${searchForm.searchContents}"/>${paramUserId}<c:out value="${searchForm.userId}"/>${paramPrefectures}<c:out value="${searchForm.prefectures}"/>${paramCities}<c:out value="${searchForm.cities}"/>${paramPetType}<c:out value="${searchForm.petType}"/>${paramPetSex}<c:out value="${searchForm.petSex}"/>">${status.index}</a></li>
         </c:when>
         <c:otherwise>
          <li id="${pageItemIDInitName}${pageLinkNumStr}" class="page-item">
          <a class="page-link"
           href="${requestURLWithParam}${pageLinkNumStr}${paramStartPage}${displayStartPageIndexNum}">${status.index}</a></li>
         </c:otherwise>
        </c:choose>
       </c:forEach>
      </c:otherwise>
